...
class LocateAnythingWorker:
    ...
    def verify_crop(self, image: Image.Image, class_name: str) -> bool:
        from .crop_verify import parse_verification_answer

        prompt = f'Does this image contain "{class_name}"? Answer only yes or no.'
        result = self._predict(image, prompt, max_new_tokens=32, temperature=0.1)
        return parse_verification_answer(result["answer"])
...
def detect(
    ...
    try:
        ...
        if crop_verification and boxes:
            from .verification_vlm import run_crop_verification

            before = len(boxes)
            backend = verification_vlm if verification_vlm in ("qwen3_vl", "locate_anything") else "qwen3_vl"
            # Free detection tensors before loading verification VLM
            gpu_mem.full_cleanup()
            boxes = run_crop_verification(img, boxes, backend)
            if before != len(boxes):
                logger.info(
                    "Crop verification (%s): dropped %d box(es), kept %d",
                    backend,
                    before - len(boxes),
                    len(boxes),
                )
